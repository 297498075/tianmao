...
namespace DBCommon.MySQL
{
    public class MySQL : IDataBase
    {
        ...
        static MySQL()
        {
            using (FileStream fs = new FileStream("MySQL.json", FileMode.Open))
            {
                using (StreamReader sr = new StreamReader(fs))
                {
                    dictionary = JsonConvert.DeserializeObject<Dictionary<string, string>>(sr.ReadToEnd());
                }
            }
            ConnectionString = "Host=" + dictionary["Host"] + ";Port=" + dictionary["Port"] + ";Username=" + dictionary["Username"] + ";Password=" + dictionary["Password"] + ";Database=" + dictionary["Database"] + ";charset=UTF8;";
        }
        ...
        public DataSet ExecuteDataSet(string sql, CommandType cmdType=CommandType.Text, Dictionary<string, object> dic = null)
        {
            ...
            using (MySqlConnection connection = GetMySqlConnection())
            {
                try
                {
                    ...
                    command = GetMySqlCommand(connection, sql, parameters.ToArray(), CommandType.Text);
        ...
        public int ExecuteNonQuery(String sql, Dictionary<string, object> dic = null, CommandType commandType = CommandType.Text)
        {
            ...
            int value = -1;
            var command = GetMySqlCommand(connection, sql, parameters.ToArray(), CommandType.Text);
        ...
        private static MySqlConnection baseConnection;
        private static MySqlCommand baseCommand;

        private static MySqlConnection GetMySqlConnection()
        {
            if (baseConnection == null) baseConnection = new MySqlConnection(ConnectionString);
        ...
        private static bool ReturnConnection(MySqlConnection mysqlConnection)
        {
        ...
        {
            if (baseCommand == null) baseCommand = new MySqlCommand();
